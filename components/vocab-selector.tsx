...
export default function VocabSelector({ conjugationData }: VocabSelectorProps) {
  ...
  useEffect(() => {
    ...
    const savedWritingSystem = localStorage.getItem(
      "writingSystem"
    ) as WritingSystem;
  ...
  useEffect(() => {
    localStorage.setItem(
      "selectedWords",
      JSON.stringify(Array.from(selectedWords))
    );
  }, [selectedWords]);
  ...
  const toggleAll = () => {
    if (isAllSelected) {
      ...
      setSelectedWords(
        new Set(conjugationData.map((item) => item.Word.dictionary.kanji))
      );
    }
      ...
      <CardContent className="space-y-6">
        <div className="space-y-4">
          ...
          <div className="flex items-center space-x-2">
            <Switch
              id="show-verb-types"
              checked={showVerbTypes}
              onCheckedChange={setShowVerbTypes}
            />
            <Label htmlFor="show-verb-types">
              Show word types (Ru-verb, U-verb, etc.)
            </Label>
          </div>
          <div className="flex items-center space-x-2">
            <Switch
              id="show-class-level"
              checked={showClassLevel}
              onCheckedChange={setShowClassLevel}
            />
            <Label htmlFor="show-class-level">
              Show class level (JPN 111, JPN 112, etc.)
            </Label>
          </div>
        </div>
          ...
          <div className="flex justify-between items-center">
            ...
            <div className="flex gap-2 flex-wrap">
              ...
              <DropdownMenu>
                ...
                <DropdownMenuContent>
                  <DropdownMenuItem onClick={() => selectByType("verb-ru")}>
                    All Ru-verbs
                  </DropdownMenuItem>
                  <DropdownMenuItem onClick={() => selectByType("verb-u")}>
                    All U-verbs
                  </DropdownMenuItem>
                  <DropdownMenuItem
                    onClick={() => selectByType("verb-irregular")}
                  >
                    All Irregular verbs
                  </DropdownMenuItem>
                  <DropdownMenuItem onClick={() => selectByType("adjective-i")}>
                    All i-adjectives
                  </DropdownMenuItem>
                  <DropdownMenuItem
                    onClick={() => selectByType("adjective-na")}
                  >
                    All na-adjectives
                  </DropdownMenuItem>
                </DropdownMenuContent>
              ...
              <DropdownMenu>
                ...
                <DropdownMenuContent>
                  <DropdownMenuItem onClick={() => selectByClass("111")}>
                    JPN 111
                  </DropdownMenuItem>
                  <DropdownMenuItem onClick={() => selectByClass("112")}>
                    JPN 112
                  </DropdownMenuItem>
                  <DropdownMenuItem onClick={() => selectByClass("113")}>
                    JPN 113
                  </DropdownMenuItem>
                  <DropdownMenuItem onClick={() => selectByClass("EX")}>
                    Extra Words
                  </DropdownMenuItem>
                </DropdownMenuContent>
              </DropdownMenu>
          ...
          <ScrollArea className="h-[400px] pr-4">
            <div className="space-y-4">
              {conjugationData.map((item) => (
                <div
                  key={item.Word.dictionary.kanji}
                ...
                >
                  <Checkbox
                    ...
                    onCheckedChange={() =>
                      toggleWord(item.Word.dictionary.kanji)
                    }
                  />
